...
RAG (Retrieval-Augmented Generation): Respostas fundamentadas estritamente nos documentos carregados, com citação direta de fonte, página e parágrafo, essencial para auditorias.
Ingestão Híbrida:
...
Extração Especializada: Integração de Amazon Textract com Comprehend Medical para identificar entidades clínicas (dosagens, princípios ativos) e relações médicas complexas.
Human-in-the-Loop: Fluxos de aprovação integrados para que informações críticas sejam validadas por especialistas antes da indexação final.
